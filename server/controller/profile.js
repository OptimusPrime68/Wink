...
exports.allProfile = (req, res) => {
  ...
  Profile.findOne({ email }, function (err, result) {
    if (result == null)
      res.status(400).json({ message: "Complete Your Profile" });
    else if (result.name == null)
      res.status(400).json({ message: "Please Update Your Name" });
    else if (result.dob == null)
      res.status(400).json({ message: "Please Update Your Date of Birth" });
    else if (result.location == null)
      res.status(400).json({ message: "Please upload your location" });
    else {
      ...
      Profile.find({
        ...
        for (var i = 0; i < success.length; i++) {
          const x = geolib.getDistance(
            ...
            {
              latitude: success[i].location.coordinates[1],
              longitude: success[i].location.coordinates[0],
            }
          );
